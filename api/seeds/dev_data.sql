-- =============================================================
-- DEV SEED DATA — À n'utiliser qu'en développement local.
-- NE PAS exécuter en production.
--
-- Usage :
--   psql $DATABASE_URL -f api/seeds/dev_data.sql
-- ou via Docker :
--   docker compose exec db psql -U pumpkin_user pumpkin_hub_dev -f /seeds/dev_data.sql
-- =============================================================
...
INSERT INTO plugins (id, author_id, name, slug, short_description, description, repository_url, license, downloads_total) VALUES
    ('b3000000-0000-0000-0000-000000000001', 'a1000000-0000-0000-0000-000000000001', 'PumpkinGuard', 'pumpkin-guard', 'Lightweight anti-cheat for Pumpkin servers.', E'# PumpkinGuard\n\nA blazing-fast anti-cheat engine built in Rust.\n\n## Features\n- Movement validation\n- Combat analysis\n- Configurable thresholds', 'https://github.com/rustcraftdev/pumpkin-guard', 'MIT', 12450),
    ('b3000000-0000-0000-0000-000000000002', 'a1000000-0000-0000-0000-000000000002', 'EconoMine', 'economine', 'Full-featured server economy with shops and auctions.', E'# EconoMine\n\nComplete economy system for Pumpkin MC.\n\n## Features\n- Virtual currency\n- Player-to-player trading\n- Admin shop configuration\n- Auction house', 'https://github.com/blocksmith42/economine', 'Apache-2.0', 8320),
    ('b3000000-0000-0000-0000-000000000003', 'a1000000-0000-0000-0000-000000000003', 'TerraCraft', 'terracraft', 'Procedural world generation with custom biomes.', E'# TerraCraft\n\nAdvanced terrain generator for Pumpkin MC.\n\n## Features\n- 12 custom biomes\n- Configurable ore distribution\n- Structure generation API', 'https://github.com/oxidebuilder/terracraft', 'GPL-3.0', 5670),
    ('b3000000-0000-0000-0000-000000000004', 'a1000000-0000-0000-0000-000000000001', 'PumpkinCore', 'pumpkin-core', 'Shared utilities and API for Pumpkin plugin developers.', E'# PumpkinCore\n\nCommon library providing shared traits, macros and utilities.\n\n## Usage\nAdd to your `Cargo.toml`:\n```toml\npumpkin-core = "0.2"\n```', 'https://github.com/rustcraftdev/pumpkin-core', 'MIT', 21300),
    ('b3000000-0000-0000-0000-000000000005', 'a1000000-0000-0000-0000-000000000002', 'ChatForge', 'chatforge', 'Advanced chat formatting and channel management.', E'# ChatForge\n\nModular chat system for Pumpkin MC.\n\n## Features\n- Custom prefixes & suffixes\n- Private channels\n- Markdown support in chat', 'https://github.com/blocksmith42/chatforge', 'MIT', 3150)
ON CONFLICT DO NOTHING;
...
INSERT INTO plugin_dependencies (id, version_id, dependency_plugin_id, version_req, is_optional) VALUES
    ('e5000000-0000-0000-0000-000000000001', 'd4000000-0000-0000-0000-000000000003', 'b3000000-0000-0000-0000-000000000004', '>=0.1.0', false),
    ('e5000000-0000-0000-0000-000000000002', 'd4000000-0000-0000-0000-000000000005', 'b3000000-0000-0000-0000-000000000004', '>=0.2.0', false),
    ('e5000000-0000-0000-0000-000000000003', 'd4000000-0000-0000-0000-000000000005', 'b3000000-0000-0000-0000-000000000005', '>=0.1.0', true),
    ('e5000000-0000-0000-0000-000000000004', 'd4000000-0000-0000-0000-000000000007', 'b3000000-0000-0000-0000-000000000004', '^0.2', false),
    ('e5000000-0000-0000-0000-000000000005', 'd4000000-0000-0000-0000-000000000011', 'b3000000-0000-0000-0000-000000000004', '>=0.1.0', false)
ON CONFLICT DO NOTHING;
